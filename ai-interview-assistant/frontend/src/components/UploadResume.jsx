...
function UploadResume({ file, setFile, uploaded, setUploaded, message, setMessage }) {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      console.error(error);
      setUploaded(false);
      // Ensure specific message if the backend rejected it or failed
      const errorMsg = error.response?.data?.error || "Failed to upload resume. Only PDF format is supported.";
      setMessage(errorMsg);
    } finally {
      setUploading(false);
    }
    ...
    <div className="glass-card">
      ...
      {!file ? (
        ...
        >
          ...
          <p className="upload-text">Drag & drop your resume, or <span style={{ color: "var(--accent-primary)", fontWeight: "600" }}>browse</span></p>
